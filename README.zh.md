...
3. **文件编辑 Diff 数值** — Write、Edit 以及 `str_replace_editor`（`str_replace` 命令）工具调用会在文件路径后显示带颜色的行数变化（如 `+10/-2`、`+10`、`-11`）。
4. **设置页** — 在 **设置 → 通用** 里管理分割标记。

所有原始渲染（Markdown、工具调用、图片、表格）都由 DSH 官方渲染器完成，插件只做文本拆分和轻量的工具调用标注。
...
- **文件编辑 Diff 数值** — 模型写文件、编辑文件，或使用 `str_replace_editor` 的 `str_replace` 命令后，工具行路径后面会显示彩色徽标：`+N` 绿色表示新增行，`-N` 红色表示删除行。
