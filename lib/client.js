...
window.__ModuleLoader__.load({
	...
	factory: function (require) {
		...
		// "发送键变 ▶"的纯函数门控（PRD FR-1）。输入全部来自会话快照；
		// 不依赖 React。图片附件在输入区 props 中无可靠信号，v0 放宽：
		// 挂图 + 空草稿 + 可继续时点 ▶ 依然走续跑（附录 B 备注）。
		var TERMINAL_KINDS = ["aborted", "error", "max-tokens", "interrupted"];

		// v1.6.1 修复主路径：快照上的 turnEnds 是 Map<turn, seq> 纯序号、无
		// reason；可判定终态改从 nodes 投影行读取，旧路径仅作兜底。
		...
		function lastTerminalKind(turnEnds) {
			...
			if (typeof turnEnds.forEach === "function" && !(turnEnds instanceof Array)) {
				list = [];
				turnEnds.forEach(function (value) {
					list.push(value);
				});
			} else if (Array.isArray(turnEnds)) {
				list = turnEnds;
			} else {
				list = Object.keys(turnEnds).map(function (k) {
					return turnEnds[k];
				});
			}
			var best = null;
		...
		exports.TERMINAL_KINDS = TERMINAL_KINDS;
		exports.lastTerminalKindFromNodes = lastTerminalKindFromNodes;
		exports.lastTerminalKind = lastTerminalKind;
...
// ── dsao/resume-button（内嵌覆盖面：发送键本体变 ▶） ──────────────────────
window.__ModuleLoader__.load({
	...
	factory: function (require) {
		...
		function ensureStyles(doc) {
			...
			// Exactly mirrors official .uV2eYG_primary Button skin:
