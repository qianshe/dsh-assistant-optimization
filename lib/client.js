...
window.__ModuleLoader__.load({
	...
	factory: function (require) {
		...
		//   - agent 回复携带任意瞬时内容（字节数、代码、路径、示例值）。草稿里的
		//     指代指的是「用户」刚才在说什么，所以用户自己的发言才是解析依据。
		...
		/**
		 * 排定同一目录下的指令候选优先级。DSH 会同时加载 AGENTS.md 和 CLAUDE.md，
		 * 但两者说的是同一件事（只是面向不同 agent），所以每个目录只保留优先的那份。
		 * README.md 不会进入会话上下文——Host 在完全没有指令文件时才兜底读它。
		 * @returns AGENTS.md 为 0，CLAUDE.md 为 1，其他为 -1。
		 */
		...
		function readInstructions(nodes) {
			...
			for (var i = 0; i < nodes.length; i++) {
				...
				for (var s = 0; s < sections.length; s++) {
					...
					// 每个目录只留一份：AGENTS.md 胜过 CLAUDE.md。
		...
		/**
		 * 只取用户自己的提问尾巴——agent 回复被刻意排除。
		 *
		 * 草稿里的指代（"那个预算"）指向的是「用户」刚才在讨论什么，所以用户自己的
		 * 发言才是解析依据。而 agent 回复携带任意瞬时内容：字节数、代码片段、路径、
		 * 示例值——改写一旦把这些粘进去，就等于陈述草稿从未主张过的事实。移除污染源
		 * 胜过再加一条规则叫模型别用手里已有的东西。
		 */
