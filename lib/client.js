...
window.__ModuleLoader__.load({
	...
	factory: function (require) {
		...
		// Hysteresis: the out-of-group render signal must be observed on two
		// consecutive scans before collapsing (transient streaming states
		// must not trigger a collapse).
		...
		function manageLatestGroup(groups) {
			...
			if (!header || !header.getAttribute || header.getAttribute("data-dsao-tg-header") !== "") { pendingSignal = null; return; }
