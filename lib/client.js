...
window.__ModuleLoader__.load({
	...
	factory: function (require) {
		...
		function manageLatestGroup(groups) {
			...
			console.log("[dsao-tg] manage: size=" + latestGroup.length + " running=" + running + " auto=" + autoState + " state=" + header.getAttribute("data-dsao-tg-state"));
			if (nextRaw) {
				console.log("[dsao-tg] next: kind=" + (nextRaw.getAttribute && nextRaw.getAttribute("data-chat-flow-kind") || "?") + " offsetH=" + nextRaw.offsetHeight + " textLen=" + (nextRaw.textContent ? nextRaw.textContent.trim().length : -1) + " transparent=" + isTransparentNode(nextRaw) + " groupable=" + (nextRaw.getAttribute && nextRaw.getAttribute("data-chat-flow-kind") === "tool-call" ? isGroupableTool(nextRaw) : "n/a"));
				var walk = nextRaw, idx = 0;
				while (walk) {
					console.log("[dsao-tg]   s[" + idx + "]: kind=" + (walk.getAttribute && walk.getAttribute("data-chat-flow-kind") || "?") + " offsetH=" + walk.offsetHeight + " textLen=" + (walk.textContent ? walk.textContent.trim().length : -1) + " trans=" + isTransparentNode(walk));
					walk = walk.nextElementSibling; idx++;
					if (idx > 5) break;
				}
			} else { console.log("[dsao-tg] next: null"); }
			if (running) {
