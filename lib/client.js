...
window.__ModuleLoader__.load({
	...
	factory: function (require) {
		...
		var CSS = [
			...
			".dsao-tg-headerIcon{flex-shrink:0;display:inline-flex;align-items:center;color:var(--dsw-alias-label-caption);margin-right:8px}",
			".dsao-tg-headerLabel{font-weight:400;color:var(--dsw-alias-label-secondary);white-space:nowrap}",
			".dsao-tg-headerSep{background:var(--dsw-alias-label-caption);border-radius:1px;flex:none;width:2px;height:2px;margin:0 8px}",
			".dsao-tg-headerCount{text-overflow:ellipsis;white-space:nowrap;min-width:0;color:var(--dsw-alias-label-tertiary);flex:auto;font-size:14px;line-height:24px;overflow:hidden}",
			...
			".dsao-tg-chevron{display:inline-block;transition:transform 180ms ease;font-size:14px;line-height:1;color:var(--dsw-alias-label-secondary)}",
		...
		function scanToolGroups(root) {
			...
			if (groups.length > 0) console.log("[dsao/tool-group] detected", groups.length, "groups, sizes:", groups.map(function(g){return g.length;}));
			var toolCallItems = root.querySelectorAll("[data-chat-flow-kind=\"tool-call\"]");
			if (toolCallItems.length > 0) console.log("[dsao/tool-group] found", toolCallItems.length, "tool-call items, groupable:", Array.prototype.filter.call(toolCallItems, isGroupableTool).length);
		...
		function startToolGroupObserver() {
			...
			console.log("[dsao/tool-group] observer starting");
			ensureStyles();
