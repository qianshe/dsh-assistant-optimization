	...
	factory: function (require) {
		...
		function loadMermaid() {
			...
			var waitReady = function(tries) {
				...
				return new Promise(function(resolve) { setTimeout(function() { resolve(waitReady(tries - 1)); }, 30); });
			};

			if (window.mermaid) {
				var ready = waitReady(100);
				if (ready !== null && ready instanceof Promise) {
					_mermaidLoaded = ready.then(function(m) {
						if (!m) return null;
						m.initialize({ startOnLoad: false, theme: "dark", securityLevel: "loose", flowchart: { useMaxWidth: false } });
						return m;
					});
				} else {
					window.mermaid.initialize({ startOnLoad: false, theme: "dark", securityLevel: "loose", flowchart: { useMaxWidth: false } });
					_mermaidLoaded = Promise.resolve(ready);
				}
				...
				s.onload = function () {
					...
					if (ready !== null && ready instanceof Promise) {
						ready.then(function(m) {
							if (!m) resolve(null);
							else { m.initialize({ startOnLoad: false, theme: "dark", securityLevel: "loose", flowchart: { useMaxWidth: false } }); resolve(m); }
						});
						return;
					}
					if (!ready) { _mermaidLoaded = null; resolve(null); return; }
					ready.initialize({ startOnLoad: false, theme: "dark", securityLevel: "loose", flowchart: { useMaxWidth: false } });
					resolve(ready);
				};
		...
		function renderMermaidBlock(el, code) {
			...
			if (!mermaid) return;
				...
				try {
					...
					}).catch(function(err) { console.warn("mermaid render failed:", err.message || err); _cleanupMermaidTemp(id); });
				} catch (e) { console.warn("mermaid render exception:", e.message || e); _cleanupMermaidTemp(id); }
			});
		}
		...
		function _mountMermaid(el, svgHtml) {
			...
			svgWrap.style.cssText = "position:absolute;left:0;top:0;transform-origin:top left;display:block;padding:12px;box-sizing:border-box;background:transparent";
...
// 窄屏占位 icon：模型配置滑杆（三条轨道 + 交错滑钮），LLM 客户端里
// 模型设置的通用形制。CSS mask + currentColor 随主题色自适应。
var MODEL_CONFIG_ICON = 'url("data:image/svg+xml,' +
  '%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 viewBox=%270 0 16 16%27%3E' +
  '%3Cg fill=%27%23000%27%3E' +
  '%3Crect x=%271%27 y=%273.2%27 width=%2714%27 height=%271.6%27 rx=%270.8%27/%3E' +
  '%3Ccircle cx=%2710.5%27 cy=%274%27 r=%272.1%27/%3E' +
  '%3Crect x=%271%27 y=%277.2%27 width=%2714%27 height=%271.6%27 rx=%270.8%27/%3E' +
  '%3Ccircle cx=%275.5%27 cy=%278%27 r=%272.1%27/%3E' +
  '%3Crect x=%271%27 y=%2711.2%27 width=%2714%27 height=%271.6%27 rx=%270.8%27/%3E' +
  '%3Ccircle cx=%278%27 cy=%2712%27 r=%272.1%27/%3E' +
  '%3C/g%3E%3C/svg%3E")'

var CSS = [
  '@container (width<=460px){',
  '  [data-dsao-model-trigger]>span{display:none!important}',
  '  [data-dsao-model-trigger]::before{content:"";display:inline-flex;width:14px;height:14px;flex:none;background:currentColor;' +
      '-webkit-mask:' + MODEL_CONFIG_ICON + ' no-repeat center/contain;mask:' + MODEL_CONFIG_ICON + ' no-repeat center/contain}',
  '}',
].join('\n')
...
// ── dsao/archive-cleanup ─────────────────────────────────────────────────
window.__ModuleLoader__.load({
	id: "dsao/archive-cleanup",
	factory: function (require) {
		var module = { exports: {} };
		var exports = module.exports;
		Object.defineProperty(exports, Symbol.toStringTag, { value: "Module" });
