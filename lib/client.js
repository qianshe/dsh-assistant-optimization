	...
	factory: function (require) {
		...
		function applyGroup(group) {
			...
			if (!headerExists) {
				...
				// Expand when the turn is live (any node is running) or when the
				// group is the trailing content of an active turn (nothing follows
				// it yet → consecutive tool calls still in flight). On a page
				// load / historical render nothing is running, so every group
				// collapses; the latest group only collapses once non-tool content
				// appears after it (see manageLatestGroup).
				var shouldExpand = isTurnLive() || isGroupRunning(group) || !hasContentAfterGroup(group);
				if (shouldExpand) {
		...
		// True when any flow item anywhere in the conversation is currently
		// streaming/executing (a running tool call or assistant step). This is
		// the discriminator between a live turn and a historical render: on a
		// page load every node is at rest, so this returns false and all groups
		// collapse; during a live turn it returns true and the latest group
		// stays expanded until non-tool content arrives after it.
		function isTurnLive() {
			if (typeof document === "undefined" || !document.querySelectorAll) return false;
			return document.querySelector('[data-state="running"]') !== null;
		}
		...
		function manageLatestGroup(groups) {
			...
			// Keep expanded while any node in the turn is still running.
			...
			// Tools done and non-tool content has arrived after the group → the
			// turn moved on; collapse now. No hysteresis needed: applyGroup keeps
			// the latest group expanded while nothing follows it, so "content
			// after" reliably means the assistant has continued past the tools.
			if (hasContentAfterGroup(latestGroup) && header.getAttribute("data-dsao-tg-state") !== "collapsed") {
		...
		function startToolGroupObserver() {
			...
			obs.observe(document.body, {
				...
				attributes: true, attributeFilter: ["data-state", "data-dsao-tg-state"]
			});
			return function () {
