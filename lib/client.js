		...
		function startToolGroupObserver() {
			...
			function hasRelevantMutation(mutations) {
				for (var i = 0; i < mutations.length; i++) {
					var m = mutations[i];
					if (m.type === "attributes" && (m.attributeName === "data-state" || m.attributeName === "data-dsao-tg-state")) return true;
					...
					if (added) {
						for (var j = 0; j < added.length; j++) {
							...
							if (node.nodeType === 1 && node.querySelector && (
								node.querySelector("[data-chat-flow-key]") ||
								node.querySelector("[data-dsao-tg-header]") ||
								node.querySelector(TURN_STATUS_SELECTOR)
							)) return true;
						}
					}
					if (removed) {
						for (var k = 0; k < removed.length; k++) {
							if (isRelevantNode(removed[k])) return true;
						}
					}
				}
				return false;
