...
window.__ModuleLoader__.load({
	...
	factory: function (require) {
		...
		/**
		 * 纯函数：从会话快照生成折叠计划。
		 * 返回 { keySet: Set<key>, folds: [{ turn, hiddenKeys, anchorKey,
		 *   reasonKind, label, runMs, headerText }] }，只含"有过程可折叠"的已完成 turn。
		 */
		...
		/**
		 * 挂在 conversation.input.right：渲染不可见锚点，
				...
				React.useEffect(function () {
					...
					function sync() {
						...
						if (!enabled) {
							...
							for (var c0 = 0; c0 < cols0.length; c0++) stripColumn(cols0[c0]);
							return;
						}
		...
		function apply(ctx) {
			...
			slots.inject("conversation.input.right", function () {
				return slots.register(
					...
					turnFold.createTurnFold(React).TurnFoldMount
				);
			});
