...
export async function searchWithContent({
  ...
  // The result IS the context: an explicit file list, each entry followed by
  // the code of its ranges (budget-limited, fresh from disk even on a cache
  // hit). No grep-keyword or config lines — they cost tokens without adding
  // signal. Diagnostics stay on the error path only.
