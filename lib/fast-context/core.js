...
export async function searchWithContent({
  ...
  if (meta) {
    ...
    parts.push("");
    let configLine = `[config] tree_depth=${meta.treeDepth}${fbNote}, tree_size=${meta.treeSizeKB}KB, max_turns=${maxTurns}, max_results=${maxResults}, timeout_ms=${timeoutMs}, brain=${meta.brain || "unknown"}`;
    if (excludePaths.length) configLine += `, exclude_paths=[${excludePaths.join(", ")}]`;
    if (meta.cache_hit) configLine += `, cache_hit=true`;
    if (meta.fallback) configLine += `, fallback: ${meta.fallback}`;
