...
/**
 * The minimal model-facing guidance. Purpose + when to prefer it + call shape.
 * Biased toward FIRST-use for exploration: an unused locator is dead weight,
 * and the inside-a-range pointer it returns is what makes one `read` cover
 * the area. Kept short on purpose: every extra clause is context tax (the
 * gate test enforces <400 chars, no code fences, no examples).
 */
const PROMPT_TEXT = [
  'Prefer context_search FIRST when exploring unfamiliar code: you know what a feature does but not which files or symbols implement it, or you need exact paths and ranges before reading.',
  'Pass a natural-language query; it returns candidate files with line ranges plus the code within them (budget-limited). Use grep/glob only when you already know an exact string or path.',
].join(' ');
...
export async function applyFastContextTool(ctx) {
  ...
  ctx.effect(() => tools.register({
    name: 'context_search',
    description:
      'Semantic code search: describe what you are looking for in natural language and get candidate files with line ranges plus the code of those ranges (budget-limited). Preferred FIRST step when exploring unfamiliar code or when the target is vague — it returns exact paths/ranges so one read covers the area; use grep/glob when you already know an exact string or path.',
    parameters: {
      type: 'object',
      properties: {
        query: {
        ...
        include_content: {
          type: 'boolean',
          description: 'Whether to include the code of the returned line ranges in the result (under a ~12KB content budget). Defaults to true; set false for a path+range list only.',
        },
      },
      required: ['query'],
    },
