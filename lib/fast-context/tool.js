...
const PROMPT_TEXT = [
  ...
  'Pass a natural-language query; it returns candidate files with line ranges plus the code within them (budget-limited). Use grep/glob instead whenever you already know an exact string or path.',
].join(' ');
...
export async function applyFastContextTool(ctx) {
  ...
  ctx.effect(() => tools.register({
    name: 'context_search',
    description:
      'Semantic code search: describe what you are looking for in natural language and get candidate files with line ranges, plus the code of those ranges (budget-limited). For vague targets where the exact file, symbol, or string is unknown.',
    parameters: {
      type: 'object',
      properties: {
        query: {
