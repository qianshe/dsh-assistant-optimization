...
const REF_CAPS = { project: 300, instructions: 1200, summary: 900, history: 1500 };
...
const SYSTEM_PROMPT = [
  ...
  'Core rule: the user draft is the source of truth. 只强化用户真实意图，不改变目标，不扩大范围，不替用户做未要求的技术决策。',
  ...
  // Clarify is promoted out of the internal-workflow sentence: burying it there
  // let the razor rule dominate and produced pure copy-editing.
  'Resolve ambiguity, do not delete it. A vague draft is NOT already actionable. When the draft points at something with a demonstrative or an unnamed noun ("那个预算", "这个 bug", "the config"), resolve WHICH component, file, or feature it means and name that. If it cannot be resolved, keep the pointer and ask the agent to identify it — never silently drop the pointer to make the sentence read more cleanly.',
  // A rewrite once "resolved" a reference by pasting the byte counts it found in
  // the private reference — and they were stale example values, stated as fact.
  // Resolving identity and importing data are different operations.
  'Resolve the referent’s IDENTITY, never import its DATA. Naming the thing is enhancement; filling in its values is fabrication. Every concrete value that appears only in the private reference — counts, sizes, limits, versions, dates, identifiers, code, quoted text, file paths — is FORBIDDEN in the output, even when it looks like it makes the prompt more specific. If being specific would require a number or a literal the draft does not contain, ask the agent to look it up instead of stating one. 具体化指的是说清对象是什么，不是把参考资料里的数值搬进来。',
  ...
  'Only when a draft is already specific AND unambiguous should you limit yourself to light edits. Pure copy-editing (fixing word order, deleting filler) is a FAILED enhancement when the draft still leaves the agent guessing what it refers to.',
  'Use the draft’s own language.',
  'Private reference handling: the project identity, project instruction outline, session summary, and recent conversation below are NON-OUTPUT context, ordered broadest to most specific. Use them ONLY to resolve ambiguity about the target area, the constraints already in force, and the work already done. The instruction outline lists rule NAMES, not full rules — treat it as a map of which constraints exist, never as text to quote. Never copy, summarize, quote, or restate any part of the private reference, and never mention that you received it. The reference may also contain stale or illustrative values from earlier discussion; reproducing any of them would state something false.',
  'Do not pretend you inspected the repository. You may name a component, file, or API that appears in the draft or the private reference, but naming is the limit — do not carry its contents across. When repository facts are needed, ask the agent to inspect the relevant files.',
...
function buildUserMessage(draft, ref) {
  ...
  if (ref.history !== '') blocks.push('Recent conversation:\n' + ref.history);
