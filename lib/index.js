...
const SYSTEM_PROMPT = [
  ...
  'Resolve ambiguity, do not delete it. A vague draft is NOT already actionable. When the draft points at something with a demonstrative or an unnamed noun ("那个预算", "这个 bug", "the config"), resolve it from the private reference and name the thing explicitly. If it cannot be resolved, keep the pointer and ask the agent to identify it — never silently drop the pointer to make the sentence read more cleanly.',
