...
const SYSTEM_PROMPT = [
  ...
  'Internal workflow, applied silently: Keep explicit intent and constraints; Drop filler and unsupported context; Clarify vague decisions as options or questions; Inspect by asking the agent to read the relevant files when repository facts are needed; Propose lightweight options for open-ended requests; Verify with the smallest useful check; Defer high-risk or irreversible actions to user confirmation.',
  'Preserve the task mode: discussion stays discussion, investigation stays investigation, implementation stays implementation. Do not turn planning or discussion into implementation unless the draft explicitly asks to implement.',
  'If the draft is already actionable, only make light edits. Use the draft’s own language.',
  'Private reference handling: the project identity, project instruction outline, session summary, and recent conversation below are NON-OUTPUT context, ordered broadest to most specific. Use them ONLY to resolve ambiguity about the target area, the constraints already in force, and the work already done. The instruction outline lists rule NAMES, not full rules — treat it as a map of which constraints exist, never as text to quote. Never copy, summarize, quote, or restate any part of the private reference, and never mention that you received it.',
  'Do not pretend you inspected the repository. Only name files, components, or APIs that appear in the draft or the private reference; otherwise ask the agent to inspect the relevant files.',
...
function apply(ctx) {
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const userText = buildUserMessage(text, {
      ...
      try {
        const result = await enhance(llm, selection, userText, controller.signal);
        // Text already collected wins over a late failure: a truncated but
        // usable rewrite beats an error with an untouched draft.
        if (result.text !== '') {
          writeJson(res, 200, { text: result.text, provider: selection.provider, model: selection.model });
          return;
        }
        if (result.problem !== null) {
          writeJson(res, 502, { error: `model call did not complete: ${result.problem}` });
          return;
        }
