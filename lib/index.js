...
//   ctx.workspaceRegistry  archived-session set + workspace accounts — archives route
//   ctx.sessionPersistence stored logs (`list()` / `locate()`) — archives route
//   ctx.sessions           live-session registry (deletion guard) — archives route
//   ctx.sessionProjectionCache  per-session checkpoint row removal — archives route
...
import { createArchiveCleanup, parseIdList, ARCHIVES_ROUTE_PATH } from './archive-cleanup.js';
...
/**
 * Archived-session cleanup surface (see lib/archive-cleanup.js for why this
 * lives in-process and what it may touch).
 *
 * GET  → read-only inventory: every archived id with its stored transcript
 *        path, size, owning workspace, and whether it is live right now.
 * POST → `{ action: 'delete' | 'restore', ids: [...], confirm: true }`.
 *        `delete` is irreversible (the transcript under `~/.dsh/sessions` is
 *        the only copy of that history) and is therefore gated on an explicit
 *        `confirm: true` in the body, on the id being in `archivedSessionIds`,
 *        and on the session not being live. `restore` only un-archives.
 *
 * Nothing here can reach a session the user has not archived, which is the
 * whole point of driving the operation off the registry's archive set instead
 * of off a directory scan.
 */
function archivesRoute(ctx) {
  return {
    kind: 'exact',
    path: ARCHIVES_ROUTE_PATH,
    handler: async (req, res) => {
      if (!isLoopback(req)) {
        writeJson(res, 403, { error: 'forbidden: loopback-only' });
        return;
      ...
      if (registry === undefined || registry === null || typeof registry.list !== 'function') {
        writeJson(res, 503, { error: 'archive cleanup needs the workspaceRegistry service' });
        return;
      }
      if (persistence === undefined || persistence === null || typeof persistence.list !== 'function') {
        writeJson(res, 503, { error: 'archive cleanup needs the sessionPersistence service' });
        return;
      }
      ...
      const cleanup = createArchiveCleanup({
        ...
        // dsh-api-remotes' connection layer listens for every emit-mode event
        // (`ctx.on('api-session/removed', …)`) and forwards it to connected
        // clients, so emitting from this context is the documented way to say
        // "this session no longer exists". The client applies it as a
        // `{kind:'remove'}` list mutation, dropping the sidebar row — without
        // it, a deleted session's row survives as a ghost that the pruned
        // archive filter no longer hides (未分组 residue, stale 运行中 dot).
        notifyRemoved: (sessionId) => ctx.emit('api-session/removed', sessionId),
      });

      if (req.method === 'GET') {
        try {
          writeJson(res, 200, await cleanup.scan());
        } catch (error) {
          writeJson(res, 500, { error: `scan failed: ${error instanceof Error ? error.message : String(error)}` });
        }
        return;
      }
      ...
      if (body === undefined) {
        writeJson(res, 400, { error: 'expected a JSON body with { action, ids }' });
        return;
      }
      const action = typeof body.action === 'string' ? body.action : '';
      const { ids, overflow } = parseIdList(body.ids);
      if (action !== 'delete' && action !== 'restore') {
        writeJson(res, 400, { error: `invalid action: expected "delete" or "restore"` });
        return;
      }
      if (ids.length === 0) {
        writeJson(res, 400, { error: 'missing ids' });
        return;
      }
      if (action === 'delete' && body.confirm !== true) {
        writeJson(res, 400, { error: 'deleting archived sessions is irreversible; resend with confirm: true' });
        return;
      }
      const force = action === 'delete' && body.force === true;

      try {
        const result = action === 'delete' ? await cleanup.remove(ids, { force }) : await cleanup.restore(ids);
        if (overflow) result.overflow = true;
        writeJson(res, 200, { action, force, ...result });
      } catch (error) {
        writeJson(res, 500, { error: `${action} failed: ${error instanceof Error ? error.message : String(error)}` });
      }
    },
  };
}
...
function apply(ctx) {
  ...
  ctx.effect(() => ctx.webServer.register(archivesRoute(ctx)), 'dsao: archive cleanup route');
