...
// Manual entry for the fast-context search tool's key. The host resolves
// keys in this order: WINDSURF_API_KEY env var → this manual entry
// (~/.dsh/dsao-windsurf-key) → local Windsurf/Devin auto-read. With no key
// at all the tool and its prompt guidance are not registered, so the status
// line doubles as the "why is the tool missing" answer.
// Changes take effect on the next restart (tool registration happens at
// profile boot); the saved value is never echoed back beyond a 4+4 preview.
...
function createWindsurfKeySetting(React) {
  function WindsurfKeySetting() {
    ...
    var leftCol = React.createElement("div", { style: Object.assign({ flex: "1 1 auto", minWidth: "0" }, titleStyle) }, "Windsurf Key");
  ...
  return WindsurfKeySetting;
}

exports.createTagsSetting = createTagsSetting;
exports.createWindsurfKeySetting = createWindsurfKeySetting;
