...
function createTurnFold(React, resetGroups, rescanGroups) {
  ...
  function TurnFoldMount(props) {
    ...
    React.useEffect(function () {
      ...
      //   · 塌缩 → 浏览器自动把 scrollTop 钳到新 floor（落点恰在 floor 上，
      //     DSH observed-top 账本把这类钳制归因为自身，follow 不脱钩）；
      //   · 净增高 → pinFloor 显式写回 floor（与 DSH toBottom 同式，落点
      //     在 floor 上，账本归因中性）。
      // 为何必须在底部时弃锚点：锚点钉住的目标是「保住正在读的内容」
      // （中部阅读语义）；底部读者若仍钉视口顶行，写回会落在新 floor 上
      // 方——converge 的 strip 先把列撑高（floor 静默下移、无 scroll
      // 事件、DSH 账本不更新），重折叠后顶行被钉住即偏离 floor 数百 px
      // 起。DSH 把偏离账本 >0.5px 的滚动一律归因为读端输入：>24px 即脱
      // 钩 follow 并把中部位置持久化进 chatScrollPositions——之后每次打
      // 开该会话 restore 中部位置取代 toBottom（「打开会话后滚动条未定
      // 位到底部」的根因）。
      ...
      // 变更前的底部状态：距底 ≤0.5px 视为在底部（与 DSH 账本的 0.5px
      // 归因容差同量级）。converge 必须在 strip 前采样（strip 撑高列后
      // floor 下移，再采样会把「本在底部」误判为中部）。
      // clientHeight=0（无布局环境 / 未渲染容器）时几何不可测 → 底部态不
      // 可判定，按中部处理（回退传统锚点补偿；对零高容器是无害 no-op）。
      ...
      // 钉底（已在底部则无写）：底部意图下「净增高」把底部读者抛离 floor
      // 时的兜底；写 H 由浏览器钳到 floor，落点恰在 floor 上，DSH 账本
      // 归因中性（floor 落点必然 ≤24px 内 → isAtBottom 恒真）。
      ...
      // converge 的锚点选择：视口顶边穿过的第一个「存活行」（不在当前
      // 计划隐藏集里的行）——折叠后仍留在视口顶部的行。判据 bottom >
      // 视口顶（与 DSH pagingAnchor 一致）：DSH 切走前 saved 位置锚定的
      // 正是该行（折叠态顶行只可能是存活行），钉住它 = 切回视图精确
      // 复现切走前状态；净零轮的 strip→refold 把它恰好移回原位，
      // delta=0 不写、视图不变。退化兜底：视口内无存活行时取首个相交
      // 行（viewportAnchor 语义）。
      ...
      // bottomIntent / preAnchors：变更前采样的 floorState 与锚点数组
      // （converge 传入 strip 前采样；锚点必须在 strip 前采样——重展开
      // 会改变视口顶边所在的行，事后重选会把钉住目标漂到更高的行上）。
      // 缺省时在 sync 内采样（此时列尚未被改动，同样即变更前态；禁用
      // 分支逐列在 strip 前采样）。
      ...
      function converge() {
        ...
        // · strip 撑高列后 floor 静默下移（无 scroll 事件、DSH 账本不更新），
        //   再采样会把「本在底部」误判为中部（见上方「底部意图」注释）；
        // · 重展开会改变视口顶边所在的行（同一 scrollTop 指向更高的内容），
        //   事后重选锚点会钉住更高的行——对已收敛列的每一轮 converge 都把
        //   中部读者的视图上漂一个过程块高度，且 DSH 账本按「读端滚动」保存
        //   漂后位置（「切回后既不在底部也不在切走前位置」的根因，逐次切换
        //   累积）。改用 strip 前的「存活行」锚点（survivingAnchor）：切回时
        //   精确复现 DSH restore 的切走前视图；净零轮 delta=0 不写、视图不变。
