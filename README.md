...
`conversation.input.right` exists, but it renders *before* the model select and the context meter, while the button belongs in the trailing cluster just left of the send button — where no slot exists. So the entry renders only a hidden anchor there, walks up to the `.trailing` container that owns the send button, and inserts a plain DOM button before the **first** button in the primary group (send alone when idle; stop+send whenever a run has an interrupt available). Anchoring to the first primary means the button never wedges between stop and send, so the stop button is not displaced. Placement follows the same discipline as the diff badge.
