...
`conversation.input.right` exists, but it renders *before* the model select and the context meter, while the button belongs immediately left of the send button — where no slot exists. So the entry renders only a hidden anchor there, walks up to the `.trailing` container that owns the send button, and inserts a plain DOM button before it, under the same placement discipline as the diff badge.
