...
**Settings → Archived sessions** — its own entry in the settings nav, not a row inside General, because this is a workflow page rather than a preference toggle — scans the archive set and reports, per session: title (from the live session list), project **name** (the full path stays one hover away), size, and last write. From there:

- **Delete selected** — purges the session directory the backend derives for that id (every stored log generation — `session.jsonl[.zstd]` and `session.vN.jsonl[.zstd]` — plus any other session-owned artifact inside it; deleting only the file `locate()` names would let an older generation resurface after a restart), removes the projection checkpoint row, detaches the session from its project account, announces `api-session/removed` so every open GUI drops the sidebar row immediately (no ghost under ungrouped, no stale "running" dot), and prunes the archive entry. Two clicks: the first arms the button, the second commits. Irreversible — the transcript is the only copy of that history, so the row also carries an **export** link (the built-in `session.export` route) to grab a ZIP first.
...
Nothing here can touch a session you have not archived: the archive set drives the candidate list, so an un-archived id is refused (`not-archived`) even under force — and a protected session is not even cancelled. Attached sessions need force to be offered at all, split by the host's own truth source into `running` (live agent executing) and `attached` (resident but idle — the earlier build mislabeled both as "live-session" and blocked them). The delete unit is the **session directory** the backend's own `locate()` derives — `<root>/<project>/<session-id>/` — and it is purged recursively, because the backend keeps one log per Session format generation in it and promotes the newest file it finds. That recursion is bounded by three checks that must all pass first: the path comes from `locate()` and never from caller input, the located basename still looks like a session log (`session(.vN)?.jsonl[.zstd]`), and the directory is named after that id. Failing any check reports `kept` and leaves the id archived (retryable, never silently un-hidden), and the `<project>` level is never touched, so sibling sessions cannot be taken.

The stored-log listing is read in both shapes it has shipped in — bare headers (older backends) and `SessionPersistenceSnapshot` envelopes carrying the header under `.header` (current ones). Reading only the first is what made every delete on a current host a no-op that still un-archived the row, resurrecting the session under 未分组 after a restart; so a listing whose entries cannot be mapped to ids at all now aborts the whole operation loudly instead of being mistaken for "no stored logs".
